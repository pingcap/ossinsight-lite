...
import { LayoutConfig, LibraryItem } from '@/utils/types/config';
...
export async function uploadLayoutJsonAction (formData: FormData) {
  const config: LayoutConfig = JSON.parse(await (formData.get('layout.json') as File).text());
...
export async function importLayoutConfig (config: LayoutConfig) {
  if (config.version === 1) {
  ...
  await withConnection(getDatabaseUri(ADMIN_DATABASE_NAME), async ({ sql, beginTransaction }) => {
    ...
    for (const [name, { items, upstream, lastSyncedAt, visibility, ...rest }] of Object.entries(config.dashboard)) {
      console.log(`import dashboard ${name}`);
      const confString = JSON.stringify(rest);
      await sql`
          INSERT INTO dashboards(name, properties)
          VALUES (${name}, ${confString})
          ON DUPLICATE KEY UPDATE properties = JSON_MERGE_PATCH(properties, ${confString})
      `;
